      ...
      body: Column(children: [
        ...
        Expanded(child: !_searched
          ? Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
              Icon(Icons.travel_explore, color: AppColors.textSecondary, size: R.isPhone(context) ? 40 : 56),
          ...
          : total == 0
            ? Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
                Icon(Icons.search_off, color: AppColors.textSecondary, size: R.isPhone(context) ? 40 : 56),
                const SizedBox(height: 14),
...
Widget _SectionHeader(String label, int count, Color color, IconData icon) =>
  Builder(builder: (ctx) => Padding(
    ...
    child: Row(children: [
      ...
      Text(label, style: TextStyle(color: color, fontSize: R.fs(ctx, 14), fontWeight: FontWeight.bold)),
      const SizedBox(width: 8),
      Container(
        ...
        child: Row(children: [
          ClipRRect(borderRadius: BorderRadius.circular(6),
            child: widget.imageUrl.isNotEmpty
              ? CachedNetworkImage(imageUrl: widget.imageUrl, width: imgW, height: imgH, fit: BoxFit.cover,
                  placeholder: (_, __) => _imgBox(imgW, imgH), errorWidget: (_, __, ___) => _imgBox(imgW, imgH))
              : _imgBox(imgW, imgH)),
          ...
          if (widget.badge != null && !isPhone)
            Container(
              ...
              child: Text(widget.badge!,
                style: TextStyle(color: widget.badgeColor ?? widget.color,
                  fontSize: 10, fontWeight: FontWeight.bold))),
        ]),
      ),
    );
  }
  Widget _imgBox(double w, double h) => Container(width: w, height: h, color: AppColors.card,
