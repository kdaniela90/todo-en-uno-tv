...
class _PlayerScreenState extends State<PlayerScreen> {
  ...
  Future<void> _initPlayer() async {
    ...
    try {
      ...
      setState(() {});
      // Ocultar barra automáticamente tras 4 segundos
      _scheduleHide();
    } catch (e) {
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: GestureDetector(
      onTap: _chewieController != null ? _onTap : null,
      ...
      child: Stack(children: [
        ...
        // Barra superior con animación fade
        if (!_hasError)
          AnimatedOpacity(
  ...
  Widget _buildError() => Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
    ...
    const Text('No se pudo reproducir el stream', style: TextStyle(color: Colors.white, fontSize: 16)),
    const SizedBox(height: 8),
    const Text('Verifica tu conexión o intenta con otro canal', style: TextStyle(color: Colors.white54, fontSize: 13)),
    ...
    return Container(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        const Padding(padding: EdgeInsets.symmetric(horizontal: 20, vertical: 12),
          child: Row(children: [
            Icon(Icons.settings, color: AppColors.celeste, size: 20), SizedBox(width: 10),
            Text('Audio y subtítulos', style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold)),
          ])),
        const Divider(color: Colors.white10, height: 1),
        if (!hasContent)
          const Padding(padding: EdgeInsets.symmetric(vertical: 28, horizontal: 24),
          ...
          if (_subTracks.isNotEmpty) ...[
            ...
            _TrackTile(track: const _TrackInfo(-1, 'Ninguno', ''), isSelected: _selSub == -1,
...
class _TrackInfo {
  final int index; final String label, language;
...
class _SectionLabel extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => Padding(
    ...
    child: Row(children: [
      ...
      Text(text, style: TextStyle(color: color, fontSize: 12, fontWeight: FontWeight.bold, letterSpacing: 0.8)),
    ]));
}
